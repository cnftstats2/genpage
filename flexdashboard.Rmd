...
gg <- ggplot(X, aes(asset_rank, price, shape = sc, customdata = link,
  ...
  labs(x = "Rank", y = "Price", color = "Market", shape = "Smart contract") +
...
> Only offers below 6000 ada are displayed. Click on the legend to display/hide hide smart contracts. Click on datapoints to open the marketplace offer.
...
gg <- ggplot(X, aes(asset_rank, price, shape = sc, customdata = link,
  ...
  labs(x = "Rank", y = "Price (log)", color = "Market", shape = "Smart contract") +
  theme(legend.title = element_text(size = 10))
...
> `Q` = Quantiles. For instance, `Q05%` means that only 5% of the offers are equal or below that price.
...
gg <- ggplot(X, aes(asset_rarity, price, shape = sc, customdata = link,
  ...
  scale_y_continuous(breaks = scale_y) +
  labs(x = "Rarity", y = "Price", color = "Market", shape = "Smart contract") +
  theme(legend.title = element_text(size = 10))
...
## Column 2 {data-width=500, .tabset}
### Cheapest offers by rarity range
```{r}
...
> Tip: Click on the image to be open the marketplace website with the offer.

### Offers stats by rarity range
```{r}
X <- DT[, .(num_assets = .N,
            min_price = min(price),
            quant_05_price = quantile(price, probs = .05, names = FALSE),
...
> Use the search box to loof for your BCRC. For example, search `#3000`.

# Buy me a coffee {data-icon="ion-coffee"}
## Column 1
### Thanks for your support!
If you want to support the effort I put on this website, any tip would be greatly appreciated!  
...
### About the website
#### The data
- Sales data is extracted from <a href="https://cnft.io/" target="_blank">cnft.io</a> and <a href="https://www.jpg.store/" target="_blank">jpg.store</a> marketplaces.
- Cnft images, rank, and rarity scores are extracted from <a href="https://bosscatrocketclub.com/" target="_blank">bosscatrocketclub.com</a>.
- The website is updated every ~30mins. This will be improved in the future. You need to hit `F5` to manually see the updates on the website.

#### Comming soon
- Only the *offers* (the data from the marketplaces) are currently analysed, but I plan also to integrate the *sales* (the data from <a href="https://opencnft.io/" target="_blank">opencnft.io</a>).

#### Disclaimer
- Please note that this is an early version, so there might be some bugs! Use at your own risk.

### About myself
- I am passionate about crypto, (c)nfts, stats, and programming!
- Contact me on discord: <a href="https://discord.com/users/K2#8332/" target="_blank">K2#8332</a>

## Columun 2
### Resources
- Find your BCRC ranking and rarity score on <a href="https://bosscatrocketclub.com/ranking/" target="_blank">bosscatrocketclub.com/ranking</a>.
