...
ggplot(X, aes(x_label, N, fill = market)) +
  ...
  labs(x = "Price range", y = "Number of listings", fill = "",
       caption = sprintf("Floor: %d ada", min(DT$price))) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = -45, vjust = +.5, hjust = 0,
        ...
        plot.caption = element_text(color = "#008B8B", vjust = +9, hjust = 0, size = 11))
```
...
X <- RAR[, .(asset_name = paste0("SpaceApe #", asset_number), asset_rank, asset_rarity)]
datatable(X, class = 'dt-right compact stripe hover',
