...
X[, img := sprintf('<img src="../img/bcrc/BCRC%s.png" class="img_html">', asset_number)]
...
# Sweep the floor {data-icon="ion-arrow-graph-up-right"}
...
ggplot(X, aes(x_label, N, fill = market)) +
  ...
  labs(x = "Price range", y = "Number of listings", fill = "Market") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = -45, vjust = +.5, hjust = 0,
                                   color = "grey50", size = 9),
        axis.text.y = element_text(color = "grey50", size = 9))
```
...
X[, img := asset_number]
X[, img := sprintf(
  '<img src="../img/bcrc/BCRC%s.png" class="img_html">', img
)]
X[, img := sprintf('<a href="%s" target="_blank">%s</br>#%s</a>', link, img, asset_number)]
...
You can also support this project by spreading the word about it to the CNFT community! :-)
